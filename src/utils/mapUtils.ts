...
let selectionChangeCallback: (selectedFeatures: Collection<Feature>) => void = () => {};

export function onSelectionChange(callback: (selectedFeatures: Collection<Feature>) => void) {
    ...
    if (vectorLayers[id]) {
        ...
        Object.keys(vectorLayers).forEach(layerId => {
            ...
            layer.setStyle(feature => {
                const isSelected = selectInteraction?.getFeatures().getArray().includes(feature as Feature);
                if (isLayerActive) {
...
export function importGeoJSON(file: File): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (event) => {
            ...
            console.log(`Imported GeoJSON as new layer: ${layerId} with name: ${file.name}`);
            resolve();
        };
...
export function initializeMap(target: HTMLElement) {
    ...
    doubleClickZoomInteraction = map.getInteractions().getArray().find(interaction => interaction instanceof DoubleClickZoom) as DoubleClickZoom;
...
export function changeMapTileLayer(layer: MapTileLayer) {
    ...
    if (baseLayer) {
        ...
        markerAndVectorLayers.forEach((existingLayer) => layers.push(existingLayer)); // Add remaining layers
    } else {
...
export function enableDrawing(type: 'Polygon' | 'Circle' | 'LineString' | 'Point' | 'Box') {
    disableDrawing();
    ...
    drawInteraction.on('drawend', (event) => {
        console.log(`${type} drawn:`, event.feature.getGeometry());
        disableDrawing();
        setTimeout(() => {
            enableFeatureSelection();
            if (doubleClickZoomInteraction) {
...
function enableFeatureSelection() {
    ...
    selectInteraction = new Select({
        condition: click,
        style: feature => {
            const isLayerActive = activeLayerId && vectorLayers[activeLayerId]?.getSource()?.hasFeature(feature as Feature);
            const isSelected = selectInteraction?.getFeatures().getArray().includes(feature as Feature);
            if (isLayerActive) {
                return isSelected ? selectedFeatureStyle : unselectedFeatureStyle;
            } else {
    ...
    selectedFeatures.on('remove', () => selectionChangeCallback(selectedFeatures));
...
export function areFeaturesSelected(): boolean {
    return selectInteraction !== null && selectInteraction.getFeatures().getLength() > 0;
}
...
export function moveToLocation(lat: number, lng: number, zoomLevel: number = 12): void {
    if (!map) {
